 ...
 * inlining its pure `#shared/*.mjs` leaves and keeping `./dig_client.js` EXTERNAL (the wasm-bindgen
 * ESM that loads dig_client_bg.wasm via import.meta.url + the runtime SRI pin — it MUST stay a
 * runtime sibling import, plain-copied to dist root + web_accessible, never inlined).
 *
 ...
 *   2. src/background/index.ts exists and imports its shared leaves via `#shared/…`, and its ONLY
 *      relative `./…` import is `./dig_client.js` (the external one);
 *   3. every `#shared/X.mjs` leaf the SW imports exists at the repo root (so esbuild can inline it);
 *   4. dig_client.js + dig_client_bg.wasm remain in EXTENSION_FILES (the SW's external import needs
 *      them at dist root);
 *   5. build.js bundles src/background/index.ts and marks ./dig_client.js external.
 *
 * Run: node --test tests/
 */
...
test('the SW is bundled from src/background/index.ts, not plain-copied as root background.js', () => {
  ...
  // (5) build.js bundles it + keeps dig_client external.
...
test('the SW imports its shared leaves via #shared/ and only ./dig_client.js relatively', () => {
  const sw = read('src/background/index.ts');

  // Its shared leaves come from #shared/… .
  const shared = new Set();
  for (const m of sw.matchAll(/from\s+['"]#shared\/([a-z0-9-]+\.mjs)['"]/gi)) shared.add(m[1]);
  assert.ok(shared.size > 0, 'expected the SW to import at least one #shared/X.mjs leaf');

  // (3) every #shared leaf exists at the repo root so esbuild can inline it.
  const missing = [...shared].filter((f) => !existsSync(join(ROOT, f)));
  assert.deepEqual(missing, [], `SW imports #shared leaves absent from the repo root: ${missing.join(', ')}`);
