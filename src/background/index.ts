...
import { resolveCrossCheckedAnchoredRoot } from '@/lib/anchored-root';
...
import { handleDexiePost, handleDexieBrowse, handleDexieResolve } from './dexie-handlers';
...
// native confirm; the extension can request but never approve. The relay wiring + message dispatch
// live in the fully-typed sibling `./app-sign-handlers` (#1464) — lifted OUT of this frozen
...
import { fetchNftMetadataJson } from './nft-metadata';
...
// All of the wiring + the connect/sign dispatch lives in the fully-typed `./app-sign-handlers`
// (#1464); this frozen SW only owns the `broadcastRuntime` injection + forwarding the messages.
const appSignHandler = createAppSignHandler({ broadcastRuntime });
...
async function resolveAnchoredRoot(endpoint, storeId) {
  const { root, trust } = await resolveCrossCheckedAnchoredRoot({
    fromEndpoint: async () => {
      ...
      return r && typeof r.root === 'string' ? r.root : null;
    },
    fromChain: () => resolveAnchoredRootFromCoinset(storeId),
  });
  if (trust === 'mismatch') {
    console.warn(
      'DIG Extension: serving endpoint reported an anchored root that disagrees with the chain for store',
      storeId, '— using the chain root; the endpoint may be compromised.',
    );
  }
  return root;
}
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  // Fully-typed dispatch lives in `./app-sign-handlers` (#1464); it returns `true` (async
  // sendResponse) when it took the message, else `false` so matching continues below.
  if (appSignHandler.handles(message.action)) {
    return appSignHandler.handle(message, sender, sendResponse);
  }
