...
### 2.1c Tip the creator + auto-tip (#379, child of #377)

Per the $DIG North Star (§6.0), the extension surfaces an OPT-IN, one-tap, dismissible way to send the
creator of the DIG resource on the active tab some $DIG. It NEVER gates or slows consuming content.

- **Home-tab prompt (`TipCreatorWidget`, `src/features/home/TipCreatorWidget.tsx`).** Mounted in the
  Home widget board; it self-hides (renders nothing) UNLESS a DIG-protocol resource is loaded on the
  active tab (a non-null capsule from the Shield ledger `getShieldLedger`, §10) AND auto-tip is NOT
  configured. It offers a $DIG amount + a one-tap **Tip** button, a "Set up auto-tip" CTA, and a
  dismiss ("Not now", session-local). It handles loading / error / success states.
- **Auto-tip preference (`AutoTipSetting`, `src/lib/autoTip.ts`).** A fullscreen-only (§145) advanced
  setting persisting an `AutoTipConfig` to the single `chrome.storage.local` key `autotip.config`
...
- **A dedicated `chia://`/URN address bar** (`data-testid="dig-toolbar-urn-input"` injected /
  ...
  - **Canonicalization on submit (#308).** An accepted `on-dig-net` submit REWRITES the URN bar's own
    text to the canonical `chia://<sub>.on.dig.net` — covering the bare `<sub>.on.dig.net`, the
    `<name>.dig` shorthand, and the already-canonical form. The bar therefore always displays the DIG
    address the user opened; it MUST NEVER be rewritten to the local node `/s/` URL the tab actually
    loads the content from (the SW navigates the tab to the node surface separately, §5.4).
...
- The HOSTED-tier chain-anchored-root walk (§5 step 6, #228) reconstructs `DataStore` singleton
  ...
  `dig_client_bg.wasm`). The walk is FAIL-CLOSED throughout (`src/offscreen/anchoredRoot.ts`): any
  missing coin record/spend, any unparsable spend (e.g. a melt), or exceeding the generation depth
  bound resolves to no root, never a guessed/stale one.
...
`MESSAGE_PROTOCOL_VERSION` `33` (#379 creator tips, §2.1c) added `tipCreator` — a one-tap $DIG tip
...
This is the canonical scan model for every wallet view; the extension MUST NOT perform a multi-index
gap-limit sweep anywhere.

The **named account selector** (`AccountSwitcher`, #95) is the SINGLE user-facing control for the
active account (#388): each account is a friendly label bookmarking one derivation index, and picking
an account sets the active index via `setActiveIndex`. The wallet exposes exactly ONE active-account
surface — there is NO separate derivation-index navigator (the prior prev/next/jump `IndexNavigator`
was pure duplication over the identical `setActiveIndex` op and is removed).
...
- **Navigation.** `setActiveIndex` (§7) sets the ACTIVE wallet's active index to an absolute,
  non-negative value (clamped). The `AccountSwitcher` (#95/#388) is the sole control that invokes it —
  picking an account passes that account's `index`. It is a PURE SW registry op — no vault round-trip, no key material involved — that
