...
/**
 * END-USER e2e for #87 + #151 — CAT auto-discovery + token metadata, driven against the BUILT
 * unpacked extension in a real browser through the real self-custody wallet + popup UI.
 *
 ...
 * monogram (no broken image), and (#151) a CAT TRANSACTION in the Activity ledger resolves to its real
 * registry ticker instead of the generic "CAT" fallback. Read-only throughout — nothing is broadcast.
 *
...
test('#151 a CAT transaction in the Activity ledger shows its REAL registry ticker, not the generic "CAT"', async () => {
  const page = await openWallet();
  await page.getByTestId('seg-activity').click();
  const row = page.getByTestId('activity-r:cat-e2e');
  await expect(row).toBeVisible({ timeout: 25_000 });
  await expect(row).toContainText('TST'); // real ticker from the registry (#151), not "CAT"
  await expect(row).not.toContainText(/\bCAT\b/);
  await page.close();
});
